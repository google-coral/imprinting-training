...
"""Builds and trains mobilenet_v1_l2norm for imprinting with options for quantization."""
...
flags.DEFINE_bool('freeze_base_model', False,
                  'Only top layers training if True')
...
def build_model():
  ...
  with g.as_default(), tf.device(
    ...
    with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope(is_training=True)):
      logits, _ = mobilenet_v1_l2norm.mobilenet_v1_l2norm(
          inputs,
          is_training=True,
    ...
    if not FLAGS.freeze_base_model:
      train_tensor = slim.learning.create_train_op(total_loss, optimizer=opt)
    else:
      trainable_variables = tf.trainable_variables()
      unfreezed_variables = [
          var for var in trainable_variables if 'Logits' in var.name
      ]
      train_tensor = slim.learning.create_train_op(
          total_loss, optimizer=opt, variables_to_train=unfreezed_variables)
